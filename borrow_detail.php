...
<?php 
    if (isset($_GET['borrow_id'])) {
        $id = $_GET['borrow_id'];
        $borrowing = getBorrowDetail($id);
        foreach ($borrowing as $borrow) {
            ?>
<div class="row">
    <div class="card mb-4">
        <h5 class="card-header">Borrow Details</h5>
        <!-- Account -->
                    ...
                    <div class="row">
                        <div class="mb-3 col-md-6">
                            ...
                            <input class="form-control" type="text" id="book" name="book"
                                value="<?= $borrow['FullName'] ?>" autofocus />
                        </div>
                        <div class="mb-3 col-md-6">
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <input type="text" class="form-control" id="Amount" name="Amount"
                                value="<?= $borrow['TotalBook'] ?>" />
                        </div>
                        <div class="mb-3 col-md-6">
                            <label for="organization" class="form-label">Borrow date</label>
                            <input type="date" class="form-control" id="Amount" name="Amount"
                                value="<?= $borrow['BorrowBook'] ?>" />
                        </div>
                        <div class="mb-3 col-md-6">
                            <label for="organization" class="form-label">Return date</label>
                            <input type="date" class="form-control" id="Amount" name="Amount"
                                value="<?= $borrow['ReturnBook'] ?>" />
                        </div>
                        <div class="mb-3 col-md-6">
                            <label class="form-label" for="country">status</label>
                            <select id="status" name="status" class="select2 form-select">
                                <option value="<?= $borrow['status'] ?>"><?= $borrow['status'] ?></option>
                            </select>
                        </div>
                    </div>
                    <div class="mt-2">
